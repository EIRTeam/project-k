...
float VehicleEngine::advance(float p_delta, float p_throttle,
  ...
  rpm += VehicleConstants::AV_TO_RPM * p_delta * engine_torque /
         settings->get_inertia();
...
void VehicleEngine::_bind_methods() {
  ClassDB::bind_method(D_METHOD("get_angular_velocity"),
                       &VehicleEngine::get_angular_velocity);
  ClassDB::bind_method(
      D_METHOD("advance", "delta", "throttle", "clutch_reaction_torque"),
      &VehicleEngine::advance);
...
float VehicleEngine::get_rpm() const { return rpm; }

Ref<VehicleEngineSettings> VehicleEngine::get_settings() const {
  return settings;
}

void VehicleEngine::set_settings(Ref<VehicleEngineSettings> p_settings) {
  settings = p_settings;
}
